...
→ Check: products/{name}/mission.md exists
```
...
→ Check: products/{name}/epics/2025-11-08-user-authentication.md exists
```
...
reviews:
  test: "PASS"
→ Check: products/{name}/reviews/user-authentication/2025-11-08-test-review.md exists
```
...
→ Check: products/{name}/reviews/validation/2025-11-08-epic-validation.md exists
```
...
→ Check: products/{name}/reviews/conflicts/2025-11-08-conflict-tech-stack.md exists
```
...
→ Check: products/{name}/standards/security-guidelines.yaml exists
```
...
For each file_path in workflow.yaml:
  If not exists(products/{name}/{file_path}):
...
```
For each file in products/{name}/**:
