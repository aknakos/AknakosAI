...
```
@products/{name}/project-brief.md (for user context)
@products/{name}/YYYY-MM-DD-ux-flow.md (for UX alignment)
@products/{name}/YYYY-MM-DD-architecture.md (for architecture alignment)
```

**Standards** (precedence: product-specific > framework defaults):
```
# Check if product-specific exists first:
@products/{name}/standards/review-checklist.yaml
